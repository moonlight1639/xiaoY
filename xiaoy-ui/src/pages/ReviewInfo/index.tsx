...
import { commitCourseComment } from "@/services";
...
function ReviewInfo() {
  const [CourseInfo, setCourseInfo] = useState<Course | null>({
    ...
    teacher: "郑慧南",
    description:
      "组合数学是数学的一个分支，研究有限或可数离散结构的性质和关系。它涉及排列、组合、图论、设计理论等多个领域，广泛应用于计算机科学、统计学和运筹学等领域。",
  });
  ...
  const handleCommitComment = () => {
    ...
    if (!CourseInfo) {
      console.error("课程信息未加载，无法提交评论");
      return;
    }
    const commit = async () => {
      const res = await commitCourseComment({
        courseId: CourseInfo.id,
        content: commentInput,
      });
      if (res.success === true) {
        setCommentInput("");
        fetchCourses(); // 提交成功后重新获取评论列表
      }
    };
    commit();
  };
      ...
      <div className="review-info-container">
        <div className="review-info-left-out">
          <div className="review-info-left">
            <img src={CourseInfo?.avatar || defaultAvator} alt={`头像`} />
        ...
        <div className="review-info-right">
          ...
          >
            <div className="review-info-right-header">
              <div>
                ...
                <p style={{ fontSize: "20px" }}>{CourseInfo?.teacher}</p>
              </div>
            </div>
            <Link
              to="/reviews"
          ...
          <div className="revie-info-comment">
            ...
            <div className="review-info-comment-container">
              <textarea
                className="review-info-textarea"
                placeholder="我们需要您的宝贵点评！"
                value={commentInput}
            ...
            {comment.map((item) => (
              <div key={item.id} className="review-info-user-comment">
                <Flex align="start" gap={10} style={{ height: "100%" }}>
                  <img
                    src={item.userAvatar || stuAvator1}
                    alt={`头像`}
                    style={{
                      width: "50px",
                      height: "50px",
            ...
            {comment.length === 0 && (
              <div className="review-info-user-comment">
                <Flex align="start" gap={10} style={{ height: "100%" }}>
                  <div style={{ width: "90%" }}>
                    <div
                      style={{
